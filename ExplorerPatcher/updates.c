                    ...
                    if (bRet || (!bRet && GetLastError() == ERROR_FILE_NOT_FOUND))
                    {
                        ...
                        if (bIsUpdateAvailable)
                        {
                            ...
                            if (!bIsUACEnabled || !dwData || (bIsBuiltInAdministratorAccount && !bIsBuiltInAdministratorInApprovalMode))
                            {
                                ...
                                WCHAR wszMsg[500];
                                swprintf_s(wszMsg, 500, Utf8Text("是否安装 %s 更新？\n\n下载地址：\n%s"), PRODUCT_NAME, wszURL2);
                                if (MessageBoxW(
...
)
{
    ...
    if (dwOperation == UPDATES_OP_INSTALL)
    {
        const wchar_t text[] =
            ...
            L"	<audio src=\"ms-winsoundevent:Notification.Default\" loop=\"false\" silent=\"false\"/>\r\n"
            L"</toast>\r\n";
        swprintf_s(buf, TOAST_BUFSIZ, text, Utf8Text("正在下载和安装更新"), Utf8Text("当前版本"), dwLeftMost, dwSecondLeft, dwSecondRight, dwRightMost);
        String2IXMLDocument(
            buf,
            wcslen(buf),
            &inputXml,
    ...
    if (IsUpdateAvailable(_T(REGPATH), hash, &bFail, wszInfoURL, MAX_PATH, hModule, &dwLeftMost, &dwSecondLeft, &dwSecondRight, &dwRightMost))
    {
            ...
            if (!bOk)
            {
                if (dwOperation == UPDATES_OP_INSTALL)
                {
                    const wchar_t text[] =
                        ...
                        L"	<audio src=\"ms-winsoundevent:Notification.Default\" loop=\"false\" silent=\"false\"/>\r\n"
                        L"</toast>\r\n";
                    swprintf_s(buf, TOAST_BUFSIZ, text, Utf8Text("更新失败"), Utf8Text("发生网络错误或安装错误。"));
                    String2IXMLDocument(
                        buf,
                        wcslen(buf),
                        &inputXml,
        ...
        else if ((dwOperation == UPDATES_OP_DEFAULT && dwUpdatePolicy == UPDATE_POLICY_NOTIFY) || (dwOperation == UPDATES_OP_CHECK))
        {
            const wchar_t text[] =
                ...
                L"	<audio src=\"ms-winsoundevent:Notification.Default\" loop=\"false\" silent=\"false\"/>\r\n"
                L"</toast>\r\n";
            LPWSTR update_guide = Utf8Text("打开 ExplorerPatcher 设置，在更新页面中选择更新。点击了解更多信息。");
            if (!dwLeftMost)
            {
                swprintf_s(buf, TOAST_BUFSIZ, text, wszInfoURL, Utf8Text("发现新版本"), update_guide);
            }
            else
            {
                WCHAR wszVersionInfo[100];
                swprintf_s(wszVersionInfo, 100, Utf8Text("发现新版本: %d.%d.%d.%d"), dwLeftMost, dwSecondLeft, dwSecondRight, dwRightMost);
                swprintf_s(buf, TOAST_BUFSIZ, text, wszInfoURL, wszVersionInfo, update_guide);
            }
            __x_ABI_CWindows_CData_CXml_CDom_CIXmlDocument* inputXml = NULL;
            String2IXMLDocument(
                buf,
    ...
    else
    {
        ...
        if (dwOperation == UPDATES_OP_CHECK || dwOperation == UPDATES_OP_INSTALL)
        {
            ...
            if(bFail)
            {
                swprintf_s(buf, TOAST_BUFSIZ, text, Utf8Text("无法检查更新"), Utf8Text("请确保网络正常，能连接到更新服务器。"));
            }
            else
            {
